...
if (response1.isNotEmpty){
  if (response33.isNotEmpty){
    ...
    final response4 = await Supabase.instance.client.from('route').select().or('disabled.is.null,disabled.not.eq.true').eq('material_route', entry['route_name']).eq('step', (response2.length + 2));
      ...
      //                                 }).eq('id', entry['idreq']);
      processList.add(response3['process']);
                                      final user = Supabase.instance.client.auth.currentUser;
  final email = user?.email;
  final response = await Supabase.instance.client.from('user').select().eq('email', email!).maybeSingle();
...
fetchfromt2 = responsey!['machine'];
  ...
  await Supabase.instance.client.from('masterdata').update({
                                        'currentprocess': processList.join(' & '),
                                      }).eq('id', entry['idreq']);
  } else {
...
if (response56?['matmov']){
  forklifter = true;
} else if (response2.isEmpty && response57?['matmov']){
forklifter = true;
}
           }
...
context.go('/dashboard');
                  ...
                  return Row(children: [
                    SizedBox(width: 20),
                      ...
                      SizedBox(width: 250, child: Text('Requested Item', style: TextStyle(fontSize: 15, fontFamily: 'Inter', fontWeight: FontWeight.bold))),
                         SizedBox(width: 20),
                      SizedBox(width: 150, child: Text('Needed', style: TextStyle(fontSize: 15, fontFamily: 'Inter',
                      fontWeight: FontWeight.bold))),
                         SizedBox(width: 20),
                      SizedBox(width: 150, child: Text('Request Time', style: TextStyle(fontSize: 15, fontFamily: 'Inter',
                      fontWeight: FontWeight.bold))),
                        
                         SizedBox(width: 20),
                          SizedBox(width: 150, child: Text('Finished Time', style: TextStyle(fontSize: 15, fontFamily: 'Inter',
                          fontWeight: FontWeight.bold))),
                           SizedBox(width: 20),
                      SizedBox(width: 200, child: Text('Total Time (min)', style: TextStyle(fontSize: 15, fontFamily: 'Inter',
                      fontWeight: FontWeight.bold))),
                         SizedBox(width: 20),
                          if (forklifter)
                          SizedBox(width: 150, child: Text('From', style: TextStyle(fontSize: 15, fontFamily: 'Inter',
                          fontWeight: FontWeight.bold))),
                             SizedBox(width: 20),
                                if (forklifter)
                          SizedBox(width: 150, child: Text('To', style: TextStyle(fontSize: 15, fontFamily: 'Inter',
                          fontWeight: FontWeight.bold))),
                      SizedBox(width: 80),
                         SizedBox(width: 20),
  ...
  final finalFilter = finalFiltern.where((entry) => username != entry['usernamed'] && entry['process'] == allowedProcess).toList();
                          ...
                          child: MouseRegion(
                            ...
                            child: Row(
                              children: [
                                  ...
                                  SizedBox(width: 250, child: Text(entry['originalneed'], style: TextStyle(fontSize: 15, fontFamily: 'Inter'))),
                                  
                                     SizedBox(width: 20),
                                  SizedBox(width: 150, child: Text(entry['neededby'] ?? 'N/A', style: TextStyle(fontSize: 15, fontFamily: 'Inter'))),
                                     SizedBox(width: 20),
                                  SizedBox(width: 150, child: Text(startTime, style: TextStyle(fontSize: 15))),
                                   
                                     SizedBox(width: 20),
                                       SizedBox(width: 150, child: Text(endTime, style: TextStyle(fontSize: 15, fontFamily: 'Inter'))),
                                               SizedBox(width: 20),
                                  SizedBox(width: 200, child: Text('$minutesElapsed', style: TextStyle(fontSize: 15, fontFamily: 'Inter'))),
                               
                               
                                  SizedBox(width: 20,),
                                       if (forklifter)
                         
                              SizedBox(width: 150, child: Text(entry['detail_from'] ?? 'N/A', style: TextStyle(fontSize: 15, fontFamily: 'Inter',
                              ))),
                          
                          
                                                       SizedBox(width: 20),
                            if (forklifter)
                                                    SizedBox(width: 150, child: Text(entry['detail_to'] ?? 'N/A', style: TextStyle(fontSize: 15, fontFamily: 'Inter',
                                                   ))),
                                                      SizedBox(width: 20,),
                                                          SizedBox(width: 43, child: Row(
                                    children: [
